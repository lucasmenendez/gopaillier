...
func main() {
	...
	var dEncryptedA = key.PubKey.Mul(encryptedA, big.NewInt(2))
	dEncryptedA = key.PubKey.Mul(dEncryptedA, encoder.Factor)

	var dInputB = new(big.Int).Mul(inputB, big.NewInt(2))
	var div = key.PubKey.Div(dEncryptedA, dInputB)
	...
	var decryptedDiv, _ = key.Decrypt(div)
	decryptedDiv = new(big.Int).Div(decryptedDiv, encoder.Factor)
	...
	var decodedDiv = encoder.DecodeFloat(decryptedDiv)
